...
export function TrendingButton({ productId, businessId, variant = 'default' }: TrendingButtonProps) {
  ...
  if (isLoading) {
    if (variant === 'action-bar' || variant === 'web-action-bar') {
      const size = variant === 'action-bar' ? 26 : 24;
      return (
        <button disabled className={`flex ${variant === 'action-bar' ? 'items-center justify-center gap-1.5 p-0.5' : 'items-center gap-1.5 min-w-[40px]'} opacity-50`}>
          <Flame size={size} className="text-gray-900" strokeWidth={1.5} />
          <span className={`${variant === 'action-bar' ? 'text-[13px]' : 'text-sm'} font-medium text-gray-900 border border-transparent whitespace-nowrap`}>...</span>
        </button>
      );
    }
    return (
  ...
  if (!trendingData?.rank) {
    if (variant === 'action-bar' || variant === 'web-action-bar') {
      ...
      return (
        <button disabled className={`flex ${variant === 'action-bar' ? 'items-center justify-center gap-1.5 p-0.5' : 'items-center gap-1.5 min-w-[40px]'} opacity-50`}>
          <Flame size={size} className="text-gray-900" strokeWidth={1.5} />
          <span className={`${variant === 'action-bar' ? 'text-[13px]' : 'text-sm'} font-medium text-gray-900 border border-transparent whitespace-nowrap`}>Trending</span>
        </button>
      );
    }
    return (
    ...
    <>
      {variant === 'action-bar' || variant === 'web-action-bar' ? (
        <button 
          ...
          className={`group flex ${variant === 'action-bar' ? 'items-center justify-center gap-1.5 p-0.5 min-w-fit' : 'items-center gap-1.5 min-w-[40px]'}`}
        >
          <Flame size={variant === 'action-bar' ? 26 : 24} className="text-orange-500 group-hover:text-orange-600 transition-colors" strokeWidth={1.5} />
          <span className={`${variant === 'action-bar' ? 'text-[13px]' : 'text-sm'} font-medium text-gray-900`}>#{trendingData.rank}</span>
        </button>
      ) : (
        <Button 
          variant="outline" 
