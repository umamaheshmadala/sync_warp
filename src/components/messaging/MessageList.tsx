...
export const MessageList = React.forwardRef<HTMLDivElement, MessageListProps>(({
  ...
  const viewModels = useMemo(() => {
    ...
    const flatViewModels = sortedMessages.map((message, index) => {
      ...
      if (dateKey !== lastDateKey && messageDate) {
        ...
        if (lastDateKey !== null && currentGroupLabel !== null) {
    ...
    console.log('🐞 [MessageList DEBUG] Data passed to Virtuoso:', {
      totalMessages: sortedMessages.length,
      groupCountsSum: groupCounts.reduce((a, b) => a + b, 0),
      groupCounts,
      groupLabels,
      lastMessages: sortedMessages.slice(-5).map(m => m.created_at)
    })
  ...
  useImperativeHandle(listHandleRef, () => ({
    ...
    scrollToBottom: (behavior: 'auto' | 'smooth' = 'smooth') => {
      if (virtuosoRef.current && viewModels.flatViewModels.length > 0) {
        virtuosoRef.current.scrollToIndex({
          index: viewModels.flatViewModels.length - 1,
          align: 'end',
          behavior
        });
      }
    }
  }), [sortedMessages, viewModels]);
