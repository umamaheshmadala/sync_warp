-- CORRECTED COMPREHENSIVE FIX for Friend Management System
-- This addresses the constraint dependency issue

-- 1. Fix RLS Policies that are causing 403 errors first
...
-- Drop the constraint first (not the index), then drop the index
...
-- 6. Verify test data is intact and show current status
...
-- Show detailed friend requests
SELECT 
    'Current Friend Requests:' as status,
    fr.status,
...
SELECT '✅ Comprehensive fix applied successfully!' as final_status;