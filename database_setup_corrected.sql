-- =========================================
-- CORRECTED FAVORITES SYSTEM DATABASE SETUP
-- =========================================
-- This version matches your actual table structure
-- Run this script in your Supabase SQL Editor to fix the column mismatches
...
-- ========================================= 
-- PART 4: DROP OLD FUNCTIONS AND CREATE CORRECTED VERSIONS
-- =========================================
...
CREATE OR REPLACE FUNCTION is_business_favorited(business_uuid uuid)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
CREATE OR REPLACE FUNCTION is_coupon_favorited(coupon_uuid uuid)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
-- Function to get user's favorite businesses with details (CORRECTED COLUMN NAMES)
...
BEGIN
    ...
    SELECT 
        b.id,
        COALESCE(b.business_name, b.name)::text as business_name,  -- Use business_name if available, fallback to name
        COALESCE(b.business_type, b.category)::text as business_type,  -- Use business_type if available, fallback to category
        b.description::text,
        b.address::text,
        b.latitude::double precision,
        b.longitude::double precision,
        COALESCE(b.average_rating, b.avg_rating, 0.0)::double precision as rating,  -- Handle both column names
...
-- Function to get user's favorite coupons with details (CORRECTED COLUMN NAMES)
...
BEGIN
    ...
    SELECT 
        ...
        bc.title::text,
        bc.description::text,
        bc.discount_type::text,
        bc.discount_value::double precision,
        bc.valid_until,
        b.id as business_id,
        COALESCE(b.business_name, b.name)::text as business_name,  -- Handle both column names
...
-- Success message
SELECT 'Database setup completed successfully! All favorites tables and functions are now available with corrected column mappings.' as status;