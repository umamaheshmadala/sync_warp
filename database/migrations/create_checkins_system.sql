-- Migration: Create check-ins system tables and functions
-- Created: 2025-01-29

-- Enable PostGIS extension if not already enabled (for geographic functions)
...
CREATE POLICY IF NOT EXISTS "Users can view own check-ins" ON public.business_checkins
    FOR SELECT USING (auth.uid() = user_id);

-- Users can create their own check-ins
CREATE POLICY IF NOT EXISTS "Users can create own check-ins" ON public.business_checkins
    FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Users can update their own check-ins
CREATE POLICY IF NOT EXISTS "Users can update own check-ins" ON public.business_checkins
    FOR UPDATE USING (auth.uid() = user_id);

-- Business owners can see check-ins for their businesses
CREATE POLICY IF NOT EXISTS "Business owners can view checkins" ON public.business_checkins
    FOR SELECT USING (
        business_id IN (
...
BEGIN
    ...
    WITH daily_checkins AS (
        ...
        AND checked_in_at >= NOW() - INTERVAL '%s days', days_back
    ...
    SELECT json_build_object(
        ...
        'growthRate', (
            ...
            previous_period AS (
                ...
                FROM public.business_checkins 
                WHERE business_id = target_business_id 
                AND checked_in_at >= NOW() - INTERVAL '%s days', days_back * 2
                AND checked_in_at < NOW() - INTERVAL '%s days', days_back
            )
            SELECT CASE 
...
BEGIN
    ...
    FROM public.businesses 
    WHERE id = target_business_id AND status = 'active';
