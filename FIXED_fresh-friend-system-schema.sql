-- FIXED FRESH FRIEND SYSTEM - Works with all PostgreSQL versions
...
-- 2. Create simple, clean friend_connections table
...
-- 3. Create unique constraint using functional index (works with all PostgreSQL versions)
CREATE UNIQUE INDEX friend_connections_unique_bidirectional 
ON public.friend_connections (LEAST(user_a_id, user_b_id), GREATEST(user_a_id, user_b_id));

-- 4. Add regular indexes for performance
...
-- 5. Enable RLS with simple, permissive policies
...
-- 6. Create helper functions with better error handling
...
BEGIN
    ...
    -- Order users consistently for unique constraint
    IF current_user_id < target_user_id THEN
    ...
    IF EXISTS (
        ...
        WHERE LEAST(user_a_id, user_b_id) = LEAST(ordered_user_a, ordered_user_b)
        AND GREATEST(user_a_id, user_b_id) = GREATEST(ordered_user_a, ordered_user_b)
    ) THEN
