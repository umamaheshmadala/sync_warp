import "jsr:@supabase/functions-js/edge-runtime.d.ts"
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2'

/**
 * Cleanup Old Messages Edge Function (Story 8.9.1)
 * 
 * Orchestrates the message retention cleanup process:
 * 1. Archive messages older than 90 days (soft delete)
 * 2. Clean up orphaned data (receipts, typing indicators, edits)
 * 3. Delete old storage files from message-attachments bucket
 * 4. Log operations to cleanup_logs table
 * 
 * Features:
 * - Dry-run mode: Pass ?dryRun=true to preview without deleting
 * - Idempotency: Skips if cleanup ran within the last hour
 * - Batched processing to avoid timeouts
 * - Comprehensive logging
 * 
 * Scheduled: Daily at 2 AM UTC via pg_cron
 */

interface CleanupResults {
  timestamp: string
  dry_run: boolean
  idempotency_skipped: boolean
  archived_messages: number
  orphaned_receipts_deleted: number
  old_typing_indicators_deleted: number
  old_edits_deleted: number
  storage_files_deleted: number
  storage_freed_mb: number
  execution_time_ms: number
  errors: Array<{
    operation: string
    error: string
  }>
}

Deno.serve(async (req) => {
  const startTime = Date.now()

  // Parse query parameters
  const url = new URL(req.url)
  const dryRun = url.searchParams.get('dryRun') === 'true'

  try {
    // Initialize Supabase client with service role key
    const supabaseUrl = Deno.env.get('SUPABASE_URL')
    const supabaseServiceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')

    if (!supabaseUrl || !supabaseServiceKey) {
      throw new Error('Missing required environment variables: SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY')
    }

    const supabase = createClient(supabaseUrl, supabaseServiceKey)

    const results: CleanupResults = {
      timestamp: new Date().toISOString(),
      dry_run: dryRun,
      idempotency_skipped: false,
      archived_messages: 0,
      orphaned_receipts_deleted: 0,
      old_typing_indicators_deleted: 0,
      old_edits_deleted: 0,
      storage_files_deleted: 0,
      storage_freed_mb: 0,
      execution_time_ms: 0,
      errors: []
    }

    console.log(`🧹 Starting message cleanup job... (dryRun: ${dryRun})`)

    // ========================================================================
    // Idempotency Check: Skip if cleanup ran within the last hour
    // ========================================================================
    if (!dryRun) {
      const { data: recentCleanup } = await supabase
        .from('cleanup_logs')
        .select('id, executed_at')
        .eq('operation', 'cleanup_old_messages')
        .eq('status', 'success')
        .gte('executed_at', new Date(Date.now() - 60 * 60 * 1000).toISOString())
        .limit(1)

      if (recentCleanup && recentCleanup.length > 0) {
        console.log('⏭️  Skipping: Cleanup already ran within the last hour')
        results.idempotency_skipped = true
        results.execution_time_ms = Date.now() - startTime

        return new Response(
          JSON.stringify(results, null, 2),
          { status: 200, headers: { 'Content-Type': 'application/json' } }
        )
      }
    }

    // ========================================================================
    // Task 1: Archive Old Messages (90+ days)
    // ========================================================================
    try {
      console.log('📦 Archiving old messages...')

      const { data: archiveResult, error: archiveError } = await supabase.rpc(
        'archive_old_messages',
        { p_batch_size: 1000, p_dry_run: dryRun }
      )

      if (archiveError) throw archiveError

      if (archiveResult && archiveResult.length > 0) {
        results.archived_messages = archiveResult[0].messages_archived || 0
        console.log(`✅ ${dryRun ? 'Would archive' : 'Archived'} ${results.archived_messages} messages`)
      }
    } catch (err) {
      console.error('❌ Archive error:', err)
      results.errors.push({
        operation: 'archive_old_messages',
        error: err instanceof Error ? err.message : String(err)
      })
    }

    // ========================================================================
    // Task 2: Clean Up Orphaned Data
    // ========================================================================
    if (!dryRun) {
      try {
        console.log('🗑️  Cleaning orphaned data...')

        const { data: cleanupResult, error: cleanupError } = await supabase.rpc(
          'cleanup_orphaned_data'
        )

        if (cleanupError) throw cleanupError

        if (cleanupResult && cleanupResult.length > 0) {
          results.orphaned_receipts_deleted = cleanupResult[0].orphaned_receipts_deleted || 0
          results.old_typing_indicators_deleted = cleanupResult[0].old_typing_indicators_deleted || 0
          results.old_edits_deleted = cleanupResult[0].old_edits_deleted || 0

          const totalCleaned =
            results.orphaned_receipts_deleted +
            results.old_typing_indicators_deleted +
            results.old_edits_deleted

          console.log(`✅ Cleaned ${totalCleaned} orphaned records`)
        }
      } catch (err) {
        console.error('❌ Cleanup error:', err)
        results.errors.push({
          operation: 'cleanup_orphaned_data',
          error: err instanceof Error ? err.message : String(err)
        })
      }
    }

    // ========================================================================
    // Task 3: Clean Up Old Storage Files (with batching)
    // ========================================================================
    try {
      console.log('📁 Cleaning old storage files...')

      const { data: storageResult, error: storageError } = await supabase.rpc(
        'cleanup_old_storage_files',
        { p_batch_size: 100, p_dry_run: dryRun }
      )

      if (storageError) throw storageError

      if (storageResult && storageResult.length > 0) {
        results.storage_files_deleted = storageResult[0].files_deleted || 0
        results.storage_freed_mb = storageResult[0].storage_freed_mb || 0

        console.log(`✅ ${dryRun ? 'Would delete' : 'Deleted'} ${results.storage_files_deleted} old files`)
        console.log(`   Storage ${dryRun ? 'would free' : 'freed'}: ${results.storage_freed_mb.toFixed(2)} MB`)
      }
    } catch (err) {
      console.error('❌ Storage cleanup error:', err)
      results.errors.push({
        operation: 'cleanup_old_storage_files',
        error: err instanceof Error ? err.message : String(err)
      })
    }

    // ========================================================================
    // Task 4: Log Operation (if not dry run)
    // ========================================================================
    if (!dryRun) {
      try {
        await supabase.from('cleanup_logs').insert({
          operation: 'cleanup_old_messages',
          records_affected: results.archived_messages +
            results.orphaned_receipts_deleted +
            results.old_typing_indicators_deleted +
            results.old_edits_deleted +
            results.storage_files_deleted,
          execution_time_ms: Date.now() - startTime,
          status: results.errors.length > 0 ? 'partial' : 'success',
          error_message: results.errors.length > 0
            ? JSON.stringify(results.errors)
            : null
        })
        console.log('📝 Logged cleanup operation')
      } catch (err) {
        console.error('❌ Logging error:', err)
        // Don't add to errors array - logging failure shouldn't affect response
      }
    }

    // ========================================================================
    // Calculate Total Execution Time
    // ========================================================================
    results.execution_time_ms = Date.now() - startTime

    // ========================================================================
    // Log Summary
    // ========================================================================
    console.log('\n📊 Cleanup Summary:')
    console.log('='.repeat(60))
    console.log(`Mode: ${dryRun ? 'DRY RUN' : 'LIVE'}`)
    console.log(`Timestamp: ${results.timestamp}`)
    console.log(`Archived Messages: ${results.archived_messages}`)
    console.log(`Orphaned Data Cleaned: ${results.orphaned_receipts_deleted +
      results.old_typing_indicators_deleted +
      results.old_edits_deleted
      }`)
    console.log(`Storage Files Deleted: ${results.storage_files_deleted}`)
    console.log(`Storage Freed: ${results.storage_freed_mb.toFixed(2)} MB`)
    console.log(`Execution Time: ${results.execution_time_ms}ms`)
    console.log(`Errors: ${results.errors.length}`)
    console.log('='.repeat(60))

    // ========================================================================
    // Return Response
    // ========================================================================
    const statusCode = results.errors.length > 0 ? 207 : 200 // 207 = Multi-Status

    return new Response(
      JSON.stringify(results, null, 2),
      { status: statusCode, headers: { 'Content-Type': 'application/json' } }
    )

  } catch (error) {
    console.error('💥 Fatal error:', error)

    return new Response(
      JSON.stringify({
        error: error instanceof Error ? error.message : 'Unknown error',
        timestamp: new Date().toISOString(),
        execution_time_ms: Date.now() - startTime
      }),
      { status: 500, headers: { 'Content-Type': 'application/json' } }
    )
  }
})
