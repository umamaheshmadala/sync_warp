-- Fix PYMK Duplicates: Strict Deduplication
-- This ensures mutual friends can NEVER appear more than once in the list
...
BEGIN
  ...
  WITH user_friends AS (
    -- Get all friends of the current user, enforcing uniqueness
    SELECT DISTINCT
      CASE
  ...
  candidates AS (
    ...
    -- We filter duplicates early here too
    SELECT DISTINCT
      f.user_id,
      f.friend_id
    FROM friendships f
  ...
  SELECT
    ...
    p.full_name,
    p.avatar_url,
    -- Count distinct unique friend IDs
    COUNT(DISTINCT pm.candidate_id) as matches_count, 
    (
        -- Calculation of mutual friends count (Total)
    ...
    (
        ...
        -- GROUP BY id ensures distinct rows before aggregation
        ...
        FROM (
            ...
            JOIN profiles mp ON mp.id = uf.friend_id
            WHERE f.status = 'active'
            GROUP BY mp.id, mp.avatar_url, mp.full_name
