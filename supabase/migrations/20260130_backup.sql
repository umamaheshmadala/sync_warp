...
BEGIN
    ...
    SELECT COUNT(*) INTO v_total_shares FROM deal_shares WHERE deal_id = p_offer_id;
    SELECT COUNT(*) INTO v_total_clicks FROM share_clicks WHERE deal_id = p_offer_id;
    ...
    WITH daily_counts AS (
        ...
        FROM deal_shares WHERE deal_id = p_offer_id GROUP BY 1
        UNION ALL
        SELECT date_trunc('day', created_at)::date as day, 0, 0, count(*) 
        FROM share_clicks WHERE deal_id = p_offer_id GROUP BY 1
    ),
    aggregated_daily AS (
    ...
    FROM (SELECT share_method, count(*) as count FROM deal_shares WHERE deal_id = p_offer_id GROUP BY 1) t;
...
-- Share Trigger
CREATE OR REPLACE FUNCTION trigger_refresh_analytics_share() RETURNS TRIGGER AS $$
BEGIN
    PERFORM refresh_offer_analytics(NEW.deal_id);
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS on_deal_share_insert ON deal_shares;
CREATE TRIGGER on_deal_share_insert AFTER INSERT ON deal_shares
FOR EACH ROW EXECUTE FUNCTION trigger_refresh_analytics_share();

-- Click Trigger
CREATE OR REPLACE FUNCTION trigger_refresh_analytics_click() RETURNS TRIGGER AS $$
BEGIN
    PERFORM refresh_offer_analytics(NEW.deal_id);
    RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS on_share_click_insert ON share_clicks;
CREATE TRIGGER on_share_click_insert AFTER INSERT ON share_clicks
FOR EACH ROW EXECUTE FUNCTION trigger_refresh_analytics_click();
