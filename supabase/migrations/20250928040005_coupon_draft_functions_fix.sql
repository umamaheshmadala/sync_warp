-- =========================================
-- FIXED DATABASE SCRIPT FOR SYNC_WARP
-- =========================================
-- This script fixes the "Failed to load drafts" error by creating missing functions
...
DROP FUNCTION IF EXISTS save_coupon_draft(UUID, TEXT, JSONB, INTEGER, UUID);
DROP FUNCTION IF EXISTS delete_coupon_draft(UUID);

-- ========================================= 
-- PART 2: CREATE COUPON DRAFT FUNCTIONS
-- =========================================
...
-- =========================================
-- PART 3: CREATE FAVORITES FUNCTIONS
-- =========================================
...
BEGIN
    ...
    SELECT EXISTS(
        ...
        WHERE user_id = user_id_val AND business_id = business_uuid
    ) INTO is_favorited;
    ...
    ELSE
        ...
        VALUES (user_id_val, business_uuid)
...
BEGIN
    ...
    SELECT EXISTS(
        ...
        WHERE user_id = user_id_val AND coupon_id = coupon_uuid
    ) INTO is_favorited;
    ...
    ELSE
        ...
        VALUES (user_id_val, coupon_uuid)
...
-- Ensure RLS is enabled for coupon_drafts
ALTER TABLE coupon_drafts ENABLE ROW LEVEL SECURITY;

-- Drop and recreate RLS policy for coupon_drafts
...
BEGIN
    ...
    RAISE NOTICE '  - get_coupon_drafts()';
    RAISE NOTICE '  - save_coupon_draft()';
    RAISE NOTICE '  - delete_coupon_draft()';
    RAISE NOTICE '  - toggle_business_favorite()';
    RAISE NOTICE '  - toggle_coupon_favorite()';
    RAISE NOTICE '';
    RAISE NOTICE 'The "Failed to load drafts" error should now be fixed!';
    RAISE NOTICE 'Test by navigating to your coupon management page.';
END $$;