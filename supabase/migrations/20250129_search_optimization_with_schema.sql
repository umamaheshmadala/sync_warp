...
ON public.favorite_deals (user_id, deal_id, created_at DESC);
  ...
  user_scores AS (
    SELECT
      ...
      (
        ...
        JOIN public.friendships f2 ON f1.user2_id = f2.user2_id
    ...
    LEFT JOIN public.users u2 ON u2.id = mu.id
  )
  SELECT *
  FROM user_scores
...
CREATE OR REPLACE VIEW public.slow_search_queries AS
SELECT
  query,
  calls,
  total_exec_time,
