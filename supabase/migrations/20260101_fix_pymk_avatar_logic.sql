-- Fix PYMK Avatar Logic: Limit mutual friends array and sort alphabetically
-- Keeps strict privacy filtering from previous migration
  ...
  SELECT
    ...
    (
        ...
        FROM (
            ...
            WHERE f.status = 'active'
            ORDER BY mp.full_name ASC -- Sort alphabetically for consistency
            LIMIT 5 -- Only return top 5 for UI performance
        ) sub_friends
    ) as mutual_friends
